...
import scala.reflect.ClassTag
 ...
 class JavaHBaseContext(@transient jsc: JavaSparkContext,
  @transient config: Configuration, val renewCredentialInterval: Long, val keyTabFile: String, val tmpHdfsConfgFile: String) extends Serializable {
  def this(jsc: JavaSparkContext, config: Configuration) = {
    this(jsc, config, 24 * 60 * 60 * 1000, null, null)
  }
  def this(jsc: JavaSparkContext, config: Configuration, keyTabFile: String) =
  this(jsc, config, 24 * 60 * 60 * 1000, keyTabFile, null)

  def this(jsc: JavaSparkContext, config: Configuration, renewCredentialInterval: Long) =
  this(jsc, config, renewCredentialInterval, null, null)

  val hbc = new HBaseContext(jsc.sc, config, renewCredentialInterval, keyTabFile, tmpHdfsConfgFile)
  ...
  def hbaseScanRDD(tableName: String,
               scans: Scan): JavaRDD[(Array[Byte], java.util.List[(Array[Byte], Array[Byte], Array[Byte])])] = {
    JavaRDD.fromRDD(hbc.hbaseScanRDD(tableName, scans))
  }

  def hbaseResultScanRDD(tableName: String,
                   scans: Scan): JavaRDD[Result] = {
    JavaRDD.fromRDD(hbc.hbaseResultScanRDD(tableName, scans))
  }
