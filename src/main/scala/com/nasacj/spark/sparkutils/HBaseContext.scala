...
import org.apache.spark.{LoggingExtended, SerializableWritable, SparkContext}
...
class HBaseContext(@transient sc: SparkContext,
                   ...
                   val renewCredentialInterval: Long,
                  val keyTabFile: String,
                   val tmpHdfsConfgFile: String) extends Serializable with LoggingExtended {

   def this(sc: SparkContext, config: Configuration, renewCredentialInterval: Long = 24 * 60 * 60 * 1000, tmpHdfsConfgFile: String = null) =
   this(sc, config, renewCredentialInterval, null, tmpHdfsConfgFile)

   def this(sc: SparkContext, config: Configuration, keyTabFile: String) =
   this(sc, config, 24 * 60 * 60 * 1000, keyTabFile, null)


  @transient var credentials = SparkHadoopUtil.get.getCurrentUserCredentials()
  @transient var tmpHdfsConfiguration:Configuration = config
  @transient val job = new Job(config)
   val curUser = UserGroupInformation.getCurrentUser.getUserName
   var clusterMode = false
   var pricipal = ""
   var keyTab = ""
   logInfo("current User : " + curUser + "****************************************")
   if(!curUser.contains("@") && (keyTabFile == null)) {
     System.setProperty("java.security.krb5.realm", "HADOOP.QIYI.COM")
     System.setProperty("java.security.krb5.kdc", "hadoop-kdc01")
     pricipal = curUser + "@HADOOP.QIYI.COM"
     keyTab = curUser + ".keytab"
     println("login principal: " + pricipal)
     println("login keyTab: " + keyTab)
   ...
   if(keyTabFile != null){
     System.setProperty("java.security.krb5.realm", "HADOOP.QIYI.COM")
     System.setProperty("java.security.krb5.kdc", "hadoop-kdc01")
     val temp = new File(keyTabFile)
     if(temp.exists()){
       pricipal = temp.getName.split("\\.")(0) + "@HADOOP.QIYI.COM"
       keyTab = keyTabFile
       println("login principal: " + pricipal)
       println("login keyTab: " + keyTab)
       UserGroupInformation.loginUserFromKeytab(pricipal, keyTab);
  ...
  HBaseSecurity.initCredentials(job)
   logInfo("current User after login " + UserGroupInformation.getCurrentUser.getUserName + "********************************")
  ...
  if (tmpHdfsConfgFile != null && config != null) {
   ...
   def renewCredentials(): Unit ={
     val flushCredentials = new Thread() {
       override def run = {
         while (true) {
           Thread.sleep(renewCredentialInterval)
           if(clusterMode){
             UserGroupInformation.loginUserFromKeytab(pricipal, keyTab);
           }
           HBaseSecurity.initCredentials(job)
           credentialsConf = sc.broadcast(new SerializableWritable(job.getCredentials()))
         }
       }
     }
     flushCredentials.setDaemon(true)
     flushCredentials.start()
   }
...
*/
    ...
    logInfo("credentials:" + credentials + "***********");
  ...
  RDD[(Array[Byte], java.util.List[(Array[Byte], Array[Byte], Array[Byte])])] = {
   ...
   def hbaseResultScanRDD(tableName: String, scan: Scan):
   RDD[Result] = {

     new HBaseResultScanRDD(sc, tableName, scan,
       broadcastedConf, credentialsConf)
   }
